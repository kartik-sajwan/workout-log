function varargout = HumanEva_Dataset_Viewer(varargin)
% HUMANEVA_DATASET_VIEWER M-file for HumanEva_Dataset_Viewer.fig
%      HUMANEVA_DATASET_VIEWER, by itself, creates a new instance of the 
%      viewr or raises the existing singleton*.
%
%      HUMANEVA_DATASET_VIEWER(dataset) will launch the viewer for a given 
...
%               % Loads the full HumanEvaI dataset
%           HumanEva_Dataset_Viewer(he_dataset('HumanEvaI','Train')); 
%               % Loads the Training data only of the HumanEvaI dataset
%           HumanEva_Dataset_Viewer(he_dataset('HumanEvaI','Validate')); 
%               % Loads the Validation data only of the HumanEvaI dataset
%           HumanEva_Dataset_Viewer(he_dataset('HumanEvaI','Test')); 
%               % Loads the Test data only of the HumanEvaI dataset
%           HumanEva_Dataset_Viewer(he_dataset('HumanEvaI','All')); 
%               % Loads the full HumanEvaI dataset (same as call without
%               % arguments
%
% See also: DATASET
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @HumanEva_Dataset_Viewer_OpeningFcn, ...
                   'gui_OutputFcn',  @HumanEva_Dataset_Viewer_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before HumanEva_Dataset_Viewer is made visible.
function HumanEva_Dataset_Viewer_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to HumanEva_Dataset_Viewer (see VARARGIN)

% Choose default command line output for HumanEva_Dataset_Viewer
...
function varargout = HumanEva_Dataset_Viewer_OutputFcn(hObject, eventdata, handles) 
...
% If Trial == '1' then it is part of the Training or Validation set and
% will contain both MoCap and Video data. By default enable viewing both
% and enable user to set otherwise.
if (SelectedTrial == '1')
    set(handles.VideoOnly,     'Value', 0);
    set(handles.MocapOnly,     'Value', 0);
    set(handles.MocapAndVideo, 'Value', 1);    
...
% If Trial == '2' then it is part of the Test set and will contain only
% Video data. There is no need to let user choose what to see in this case.
if (SelectedTrial == '2')
    set(handles.VideoOnly,     'Value', 1);
    set(handles.MocapOnly,     'Value', 0);
    set(handles.MocapAndVideo, 'Value', 0);
    set(handles.MocapOnly,     'Visible', 'off');
    set(handles.MocapAndVideo, 'Visible', 'off');
    set(handles.VideoOnly,     'Visible', 'on');
end

% If Trial == '3' then it is part of the Training set and will contain only
% MoCap data. There is no need to let user choose what to see in this case.
if (SelectedTrial == '3')
    set(handles.VideoOnly,     'Value', 0);
    set(handles.MocapOnly,     'Value', 1);
    set(handles.MocapAndVideo, 'Value', 0);
    set(handles.MocapOnly,     'Visible', 'on');
    set(handles.MocapAndVideo, 'Visible', 'off');
    set(handles.VideoOnly,     'Visible', 'off');
end
...
ListOfAxisHandles = [handles.AxisC1, handles.AxisC2, handles.AxisC3, ...
                     handles.AxisBW1, handles.AxisBW2, handles.AxisBW3, handles.AxisBW4];
        ...
        if (~ValidPose)
            ...
            for A = 1:length(ListOfAxisHandles)
                ...
                if (A < 4)
                    ...
                    calfilename = ['../' SelectedSubject '/Calibration_Data/BW' num2str(A-3) '.cal'];
                end               
